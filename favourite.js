...
export default class Favourite extends Component {
    ...
    render() {
        ...
        if (this.state.currText == '') {
            filterearr = this.state.movies
        } else {
            filterearr = this.state.movies.filter((movieObj) => {
        ...
        if (this.state.currgen != "All Genres") {
            filterearr = this.state.movies.filter((movieObj) => genreids[movieObj.genre_ids[0]] == this.state.currgen)
        }

        let pages = Math.ceil(filterearr.length / this.state.limit);
        let pagesarr = [];
        ...
        filterearr = filterearr.slice(si, ei);
